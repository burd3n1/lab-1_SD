...

{
     ...
     * найти определенную вещь и куда ее надо потом положить, и никто кроме неё
    ...
    {
        Rect r1(2, 5, 3, 1);
        std::cout << "Width: " << r1.getWidth() << ", Height: " << r1.getHeight() << ", Square: " << r1.getSquare() << '\n';
        printRect(r1);
        r1.setWidth(4);
        r1.setHeight(2);
        printRect(r1);
     ...
     * станет меньше 50%.
     *еализуйте в классе методы, которые позволят псевдокоду в следующем
     * блоке превратиться в настоящий код и решить задачу.
     */

    /* {
        Barrel alch(...); // бочка со спиртом
        Barrel water(...);
    ...
    
    {
        ...
        Barrel alch(capacity, capacity, capacity * 0.96); 
        Barrel water(capacity, capacity, 0.0);

        int iteration = 0;
        while (alch.getConcentrationAl() > 50.0) {
            water.pourFrom(alch, 1.0);
            alch.pourFrom(water, 1.0);
            iteration+=1;
        }
        std::cout << "After " << iteration << " iterations concentration in alcohol barrel is "
        << alch.getConcentrationAl() << std::endl;
    }
...

    {
        ...
        B.addInPlace(A); // B = B + A
        B.negate(); // B = -B
        std::cout<< B.get(0,0) <<std::endl;


        Matrix A1(2, 2, 0.0);
        A1.set(0,0,1); A1.set(0,1,2);
        A1.set(1,0,3); A1.set(1,1,4);
        Matrix B1 = A1.multiply(A1);
        std::cout << B1.get(0, 0) << " 1*1 + 2*3" << std::endl;
    }
     ...
     */

     {
        ...
        s1.setNewString("New string s1");
        std::cout << "s1 after setNewString: "; s1.print();

        //std::cout << "intput str: ";
       // s2.readLine();
        std::cout << "your str: "; s2.print();

        s1 = s2;  // Проверка оператора =
        std::cout << "s1 after s1 = s2: "; s1.print();
        return 0;
    } 
